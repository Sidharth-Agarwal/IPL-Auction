// src/components/admin/TeamForm.jsx
...
import { validateTeamForm } from '../../utils/validators';
import { DEFAULT_SETTINGS } from '../../utils/constants';
import { useNotification } from '../../context/NotificationContext';

const TeamForm = ({ teamId = null, onSuccess = null }) => {
  const initialFormData = {
    name: '',
    owner: '',
    wallet: DEFAULT_SETTINGS.DEFAULT_WALLET_AMOUNT,
    logo: ''
  };
    ...
    const loadTeam = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          wallet: teamData.wallet || DEFAULT_SETTINGS.DEFAULT_WALLET_AMOUNT,
          logo: teamData.logo || ''
        });
        setIsEditing(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showError(isEditing ? 'Failed to update team' : 'Failed to create team');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = () => {
    if (isEditing) {
      // If editing, reset to original team data
      // This would require keeping track of original data
      // For simplicity, we'll redirect or show a confirmation
      if (window.confirm('Are you sure you want to discard changes?')) {
        // Reload team data or redirect
      }
    } else {
      // Clear form
      setFormData(initialFormData);
        ...
        <div>
          ...
          {formErrors.wallet ? (
            ...
            <p className="mt-1 text-sm text-gray-500">
              Default wallet amount is {DEFAULT_SETTINGS.DEFAULT_WALLET_AMOUNT} credits
            </p>
          )}
        </div>
